...
function clicMelanger() {
    ...
    melangeEnCours = true;
    document.body.className = null;
    dom_controle.innerHTML = ''; // Supprimez le bouton d'arrêt manuel
    afficherSurConsole("Mélange en cours...");
    let melangeCount = 0; // Compteur de mélanges effectués
    timer = setInterval(function () {
        melanger();
        melangeCount++;
        if (melangeCount >= 1000) {
            clearInterval(timer); // Arrêtez le mélange après 200 mélanges
            melangeEnCours = false; // Mettez fin au mélange
            actualiserPieces(); // Mettez à jour l'affichage des pièces après le mélange
